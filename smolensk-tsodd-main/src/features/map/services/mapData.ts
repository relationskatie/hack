...
// Функция для получения объектов карты через API
export const getMapObjects = async (params?: {
  ...
  try {
    const response = await MapApi.getObjects(params);
...
// Синтетические данные объектов инфраструктуры Смоленска (fallback)
